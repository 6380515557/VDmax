...
COOKIES_FILE_PATH = os.getenv("COOKIES_FILE_PATH", "./youtube_cookies.txt")  # Set your cookies file path here
...
class DownloadURLRequest(BaseModel):
    url: str = Field(..., example="https://www.youtube.com/watch?v=dQw4w9WgXcQ")
    quality: Optional[str] = Field("bestvideo+bestaudio/best", example="22")
...
async def get_video_info(request: VideoInfoRequest):
    ydlopts = {
        ...
        "cookiefile": COOKIES_FILE_PATH,  # Cookie file to authenticate YouTube
    }
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to fetch video info: " + str(e))
...
async def get_download_url(request: DownloadURLRequest):
    ...
    ydlopts = {
        ...
        "cookiefile": COOKIES_FILE_PATH,  # Cookie file path for YouTube auth
    }
    ...
    try:
        with yt_dlp.YoutubeDL(ydlopts) as ydl:
            ...
            # Direct url for requested format
            if "url" in info:
        ...
        raise HTTPException(status_code=500, detail="Failed to get download URL: " + str(e))
